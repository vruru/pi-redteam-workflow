...
Malware employs various techniques to detect and evade automated sandbox analysis environments. Key evasion categories:

### Timing-Based Evasion
- **Sleep bombing**: Call `Sleep()` or `NtDelayExecution()` with large delays to outwait sandbox timeouts
- **Timestamp checks**: Use `rdtsc` or `QueryPerformanceCounter()` to detect accelerated virtual time
- **Execution timing**: Measure time between operations, abort if too fast (sandbox acceleration) or too slow (debugger)

### Environment Detection
- **Hardware checks**: CPU core count, RAM size, disk size (VMs typically have limited resources)
- **VM artifacts**: Registry keys, MAC address prefixes (00:0C:29 for VMware, 08:00:27 for VirtualBox)
- **Process/module scanning**: Look for VMware tools, VirtualBox guest additions, sandbox processes
- **Username/computer name**: Check for default sandbox names (USER, SANDBOX, MALWARE)

### Human Interaction Checks
- **Mouse movement monitoring**: Track cursor position changes over time
- **Window count**: Desktops with few windows likely automated
- **Recent document count**: Empty recent docs list suggests non-user environment
- **Browser history**: Check for browsing activity before executing payload

### Anti-Analysis API Patterns
- `NtQuerySystemInformation` with `SystemProcessorPerformanceInformation`
- `GetSystemInfo` to check processor count
- `GlobalMemoryStatusEx` to check RAM
- `DeviceIoControl` with `IOCTL_STORAGE_QUERY_PROPERTY` to check disk type

### Sandbox Evasion Countermeasures
- Configure CAPE/Cuckoo with realistic hardware profiles (4+ CPUs, 8GB+ RAM)
- Use CAPE's debugger module to patch sleep calls and timing checks
- Pre-populate VM with realistic user activity artifacts
- Use full-system emulation (QEMU/KVM) instead of user-mode sandboxes
