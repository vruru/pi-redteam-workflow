...
> 来源：先知《GOT/PLT 惰性绑定与 setuid 程序导入函数劫持》（xz.aliyun.com/news/92637，含可复现
> 实验全流程）。两个边界概念先分清：

- **lazy binding 影响符号解析时机**（首次调用解析 vs 启动期提前解析）；
- **RELRO 影响重定位后的页面权限**（GOT 相关页是否仍可写）。
- 因此 `-z now` 只解决"运行期首次解析"，**没有 RELRO 时 GOT 槽仍可写**——checksec 里
  `BIND_NOW` 与 `RELRO` 两列要分开判。

### 劫持成立条件（两条件同时出现）

1. 进程内存在**可控写入缺陷**（任意写/越界写/格式化字符串 %n/错误指针写）；
2. `.got.plt` 的导入函数槽在业务逻辑阶段**仍可写**（无 Full RELRO）。

setuid 放大面：euid=root 后 glibc 进入 secure-execution（AT_SECURE），过滤 LD_PRELOAD 等
环境变量——**但那只收紧"加载环境"，不审计程序自身内存写**：GOT/函数指针表/虚表/回调表照写不误。

### 劫持选型与实操要点

- 目标配对首选**同 ABI 传参**的函数对：`puts() → system()`（x86_64 System V 首参都在 rdi，
  源码 `puts(argv[4])` 被改向后参数自然继承）；目标未导入 system 时须换可用入口或先泄漏算 libc 地址；
- 提取两值（**必须同一次构建**，重编译后重提）：`readelf -r` 拿 `puts@got.plt`（JUMP_SLOT 行）；
  反汇编拿 `system@plt` 入口；`-no-pie` 下两者稳定可直接用；
- 保留桩的坑：未被调用的导入符号会被链接器剔除——用 `retain_system_plt()` 类死代码保留
  system@plt；验证桩形态（push 序号/共享 PLT0）确认槽位未被剔；
- 弱保护构建复现选项全套：`-no-pie -fno-pie -Wl,-z,lazy -Wl,-z,norelro -fcf-protection=none`；
- 分析判据（审样本时）：`checksec` 见 No RELRO / Partial RELRO + 存在写入原语 → GOT 劫持面成立，
  进利用设计；Full RELRO 则转函数指针表/虚表/回调表方向。

### 来源

- [先知 — GOT/PLT 惰性绑定与 setuid 程序导入函数劫持](https://xz.aliyun.com/news/92637)
