...
### 驱动回调枚举（进程/线程/注册表/对象通知）

Rootkit 与部分安全/监控驱动不靠 IRP，而靠**内核回调**挂在系统事件上，静态逆向需专门枚举：

| 回调类型 | 注册 API | 回调结构 | 逆向关注 |
|---|---|---|---|
| 进程/线程回调 | `PsSetCreateProcessNotifyRoutine(Ex)` | `EX_CALLBACK_ROUTINE_BLOCK` | 监控/篡改进程创建（EDR/rootkit 常用） |
| 注册表回调 | `CmRegisterCallback(Ex)` | `EX_CALLBACK_ROUTINE_BLOCK` | 隐藏/篡改注册表 |
| 对象回调 | `ObRegisterCallbacks` | `OB_CALLBACK_REGISTRATION` | 保护进程/句柄 |
| 镜像加载回调 | `PsSetLoadImageNotifyRoutine(Ex)` | `EX_CALLBACK_ROUTINE_BLOCK` | 监控/篡改 DLL 加载 |
| Minifilter | `FltRegisterFilter` | `FLT_REGISTRATION` | 文件系统过滤 |

分析流程：

```text
1. 静态：DriverEntry 里搜上述注册 API，定位回调函数地址。
2. 回调函数参数（进程回调的 PCREATE_PROCESS_NOTIFY_INFO / 注册表回调的 REG_NOTIFY_CLASS）
   → 还原「在什么事件、做什么篡改」。
3. 动态：WinDbg 枚举回调链表（如 !object、!callback）验证注册状态。
```

判据：识别驱动挂的每一类回调 + 对应 handler + 篡改语义（隐藏/保护/监控）。

### 驱动签名绕过分析（逆向视角）

Windows 64 位强制驱动签名（内核模式代码完整性 / KMCI）。分析「签名校验与绕过面」：

```text
1. 判定签名状态：静态看 Authenticode 签名（sigcheck/pefile），内核看 CI（Code Integrity）策略。
2. 绕过分析面（逆向视角，只分析不利用）：
   - 签名校验 bug：某些老 Windows 版本只验「签名存在」不验「证书链」。
   - 易受攻击的签名驱动（BYOVD）：合法签名但含漏洞的驱动（LOLDrivers 清单），
     被利用来加载未签名代码 —— 逆向点 = 其 IOCTL 漏洞（见上文 IOCTL 分析）。
   - 测试签名 / 禁用完整性检查：bcdedit /set TESTSIGNING ON（需管理员，仅测试环境）。
3. 判据：能说明「该驱动签名是否有效 + 是否存在 BYOVD 类 IOCTL 漏洞」，
   利用验证交 pentest/attack-defense（本模式只到「发现」）。
```

### 内核调试符号专项

```text
1. 符号源：Microsoft Symbol Server（.pdb），srv*https://msdl.microsoft.com/download/symbols。
2. WinDbg：.sympath srv*<本地缓存>*https://msdl.microsoft.com/download/symbols
   .reload /f driver.sys    # 加载驱动符号
3. IDA：File → Load PDB → 从符号服务器取（需符号路径配置）。
4. 符号后分析：函数名/类型直接可读，驱动逻辑还原显著加速。
```

判据：符号加载后 `!analyze`/反编译出现真实函数名与结构体类型，而非 `sub_140001000`。
