...
> **安装状态（Pi）**：本包当前只启用了这一个入口技能。41 个 `competition-*` 赛道技能已移到仓库内 `../../.disabled/`（Pi 不扫 `.` 开头的目录）以节省常驻上下文，因此 `$competition-*` 暂时不可直接调用；需要时跑 `../../toggle-ctf.sh on` + `/reload`，或直接读 `references/router-matrix.md` 后用 `read ../../.disabled/<track>/SKILL.md` 取全文。
