...
> **路径约定（Pi 安装版）**：本技能目录是 `skills/reverse-skill-router/`，路由器所需脚本与配置位于其上一级 `../`（`../scripts/`、`../config/routing.json`、`../ops/`、`../references/`），各模块技能位于 `../<module>/SKILL.md`，CTF 赛道技能位于 `../../CTF-Sandbox-Orchestrator/<track>/SKILL.md`。读取这些文件时请带上 `../` 前缀。
...
| **DSL 虚拟机逆向** | `../dsl-vm-reverse/` | JS 自定义指令集 VM（IIFE + switch-case opcode）；风控/验证码引擎等 |
