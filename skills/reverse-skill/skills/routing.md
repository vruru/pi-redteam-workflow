...
| WASM / Python bytecode / .NET / **DSL VM / 自定义虚拟机** | `dsl-vm-reverse/SKILL.md` — IIFE + switch-case opcode JS VM | `reverse-engineering/languages.md` — real WASM binaries |
...
| User Says | Route To |
|-----------|----------|
| "DSL VM / 自定义指令集 / 风控引擎逆向" | `dsl-vm-reverse/SKILL.md` — IIFE + switch-case opcode |
| "fireye / fireyejs / getToken 逆向" | `dsl-vm-reverse/SKILL.md` — runtime capture |
| "582KB JS 文件不是 WASM / 大 JS 文件逆向" | `dsl-vm-reverse/SKILL.md` — classify DSL VM first |
...
| "SRC hunting / Bug Bounty" | `src-hunter/SKILL.md` — 19 playbooks + H1 cases |
| "WAF bypass" | `src-hunter/references/payloader/` — 263 bypass steps |
